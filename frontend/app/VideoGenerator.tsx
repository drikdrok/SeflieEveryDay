...
const VideoGenerator: React.FC<VideoGeneratorProps> = ({ images, imageInfo, frameRate, readyToGenerate }) => {
    ...
    const generateVideo = async () => {
        ...
        for (let i = 0; i < images.length; i++) {
            ...
            if (newHeight % 2 == 1) // Make sure the height is even
                newHeight -= 1;
        ...
        await ffmpeg.exec([
            '-y',
            '-framerate', String(frameRate), // Frames per second
    ...
    return (
        <div className="flex flex-col items-center">
            ...
            {readyToGenerate &&
                ...
                > {isLoading ? "Loading..." : "Generate Video"}</button>
            }
            {downloadLink && (
                <a  style={{
                    padding: '1rem 2rem',
                    ...
                    borderRadius: '8px',
                    margin: '1rem',
                }}href={downloadLink} download="generated_video.mp4">
                    Download Video
                </a>
            )}
            <p ref={messageRef}></p>
